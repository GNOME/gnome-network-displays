...
static void
on_sink_list_model_items_changed_cb (gpointer    user_data,
                                     guint       position,
                                     guint       removed,
                                     guint       added,
                                     GListModel *list)
{
  NdWindow *self = (NdWindow *) user_data;

  self->find_sink_list_model = ND_SINK_LIST_MODEL (list);
}
...
gnome_nd_window_init (NdWindow *self)
{
  ...
  g_signal_connect_object (self->find_sink_list_model,
                           "items-changed",
                           G_CALLBACK (on_sink_list_model_items_changed_cb),
                           self,
                           G_CONNECT_SWAPPED);
